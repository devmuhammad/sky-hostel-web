...
export function sanitizeStudentData(data: any): any {
  ...
  try {
    return {
      ...
      first_name: data.first_name ? sanitizeName(data.first_name) : undefined,
      last_name: data.last_name ? sanitizeName(data.last_name) : undefined,
      email: data.email ? sanitizeEmail(data.email) : undefined,
      phone: data.phone ? sanitizePhone(data.phone) : undefined,
      date_of_birth: data.date_of_birth
        ? sanitizeString(data.date_of_birth)
        : undefined,
      address: data.address ? sanitizeString(data.address) : undefined,
      state_of_origin: data.state_of_origin
        ? sanitizeString(data.state_of_origin)
        : undefined,
      lga: data.lga ? sanitizeString(data.lga) : undefined,
      marital_status: data.marital_status
        ? sanitizeString(data.marital_status)
        : undefined,
      religion: data.religion ? sanitizeString(data.religion) : undefined,

      // Academic Information
      matric_number: data.matric_number
        ? sanitizeMatricNumber(data.matric_number)
        : undefined,
      course: data.course ? sanitizeString(data.course) : undefined,
      level: data.level ? sanitizeString(data.level) : undefined,
      faculty: data.faculty ? sanitizeString(data.faculty) : undefined,
      department: data.department ? sanitizeString(data.department) : undefined,

      // Next of Kin Information
      next_of_kin_name: data.next_of_kin_name
        ? sanitizeName(data.next_of_kin_name)
        : undefined,
      next_of_kin_phone: data.next_of_kin_phone
        ? sanitizePhone(data.next_of_kin_phone)
        : undefined,
      next_of_kin_email: data.next_of_kin_email
        ? sanitizeEmail(data.next_of_kin_email)
        : undefined,
      next_of_kin_relationship: data.next_of_kin_relationship
        ? sanitizeString(data.next_of_kin_relationship)
        : undefined,

      // Accommodation Information
      block: data.block ? sanitizeString(data.block) : undefined,
      room: data.room ? sanitizeString(data.room) : undefined,
      bedspace_label: data.bedspace_label
        ? sanitizeString(data.bedspace_label)
        : undefined,

      // System Information
      payment_id: data.payment_id ? sanitizeString(data.payment_id) : undefined,
      room_id: data.room_id ? sanitizeString(data.room_id) : undefined,

      // File Storage
      passport_photo_url: data.passport_photo_url
        ? sanitizeString(data.passport_photo_url)
        : undefined,
    };
  } catch (error) {
    throw new Error(
...
export function sanitizePaymentData(data: any): any {
  ...
  try {
    return {
      ...
      email: data.email ? sanitizeEmail(data.email) : undefined,
      phone: data.phone ? sanitizePhone(data.phone) : undefined,
      amount: data.amount ? sanitizeNumber(data.amount) : undefined,
    };
  } catch (error) {
    throw new Error(
