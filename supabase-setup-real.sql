-- =====================================================
-- Sky Student Hostel - REAL Database Setup
...
('sarah.wilson@example.com', '08076543210', 219000.00, 'SKY-TEST-004', 'completed', NOW() - INTERVAL '2 days');

-- Insert sample student data with real room assignments
...
(
  'Sarah', 'Wilson', 'sarah.wilson@example.com', '08076543210', '2001-03-10',
  '789 Campus Road, Abuja', 'FCT', 'Abuja Municipal', 'Single', 'Christianity',
  'MED/2021/003', 'Medicine', '200 Level', 'Medicine', 'Medicine and Surgery',
  'David Wilson', '08033333333', 'david.wilson@example.com', 'Father',
  'C', 'C2', 'Bed 3 (Top)', 
  (SELECT id FROM payments WHERE email = 'sarah.wilson@example.com')
);

-- Update room availability after student assignments
UPDATE rooms SET available_beds = array_remove(available_beds, 'Bed 1 (Top)') WHERE name = 'A1' AND block = 'A';
UPDATE rooms SET available_beds = array_remove(available_beds, 'Bed 2 (Down)') WHERE name = 'B5' AND block = 'B';
UPDATE rooms SET available_beds = array_remove(available_beds, 'Bed 3 (Top)') WHERE name = 'C2' AND block = 'C';
...
SELECT 
  ...
  s.block || s.room as room,
  s.bedspace_label,
  r.bed_type,
  p.status as payment_status
FROM students s
JOIN payments p ON s.payment_id = p.id
JOIN rooms r ON s.block = r.block AND s.room = r.name
ORDER BY s.block, s.room;
